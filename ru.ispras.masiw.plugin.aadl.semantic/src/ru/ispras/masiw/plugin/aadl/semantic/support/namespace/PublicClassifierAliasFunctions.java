...
public class PublicClassifierAliasFunctions implements Usefull{
	...
	public static void uniqueClassifierAlias(IExtendedEObject p, IExtendedEObject c,
		...
		if (alias.getClassifier().eIsProxy()) {
			...
			if (!reference.contains("::")) {
				...
				} else if (!NamespaceModel.localPublicImports.get(curPackage.getIdentifier()).containsKey(new AADLIdentifier(packageName))) {
			...
			if (NamespaceModel.localPublicNamespaces.get(curPackage.getIdentifier()).containsKey(id) || 
				...
				NamespaceModel.localPublicClassifierAliases.get(curPackage.getIdentifier()).put(alias.getIdentifier(), alias);
			}
		}
	}
