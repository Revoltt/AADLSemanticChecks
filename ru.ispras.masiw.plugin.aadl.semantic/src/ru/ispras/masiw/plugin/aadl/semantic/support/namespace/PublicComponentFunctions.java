...
public class PublicComponentFunctions implements Usefull{

	public static void publicUniqueComponentName(IExtendedEObject p, IExtendedEObject c, IExtendedEObject unused1, AADLIdentifier unused2) {
		PackageImpl curPackage = (PackageImpl) p;
		AADLDeclaration component = (AADLDeclaration) c;
		if (NamespaceModel.localPublicNamespaces.get(curPackage.getIdentifier()).containsKey(component.getIdentifier())) {
			...
			NamespaceModel.localPublicNamespaces.get(curPackage.getIdentifier()).put(component.getIdentifier(), component);
